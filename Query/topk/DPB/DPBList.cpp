...
void DPBFRIterator::TryGetNext(unsigned int k) {
  // get first
 ...
 * @param fq_pointer
 */
void DPBFRIterator::Insert(unsigned int k,
                           TYPE_ENTITY_LITERAL_ID fq_id,
                           std::shared_ptr<DPBList> fq_pointer,
...
double DPBFRIterator::DeltaCost(std::shared_ptr<DPBList>& node_pointer, int index) {
  auto delta =  node_pointer->pool_[index].cost - node_pointer->pool_[index-1].cost;
  return delta;
}

bool DPBFRIterator::NextEPoolElement(unsigned int k, std::shared_ptr<DPBList>& node_pointer, unsigned int index) {
 ...
 */
void DPBFRIterator::GetResult(int i_th, std::shared_ptr<std::vector<TYPE_ENTITY_LITERAL_ID>> record,
...
DPBFRIterator::DPBFRIterator() {
  //this->type_predicates_=std::make_shared<NodeOneChildVarPredicates>();
}

void DPBOWIterator::TryGetNext(unsigned int k) {
}

void DPBOWIterator::Insert(unsigned int k,
...
void DPBOWIterator::GetResult(int i_th, std::shared_ptr<std::vector<TYPE_ENTITY_LITERAL_ID>> record,
                              NodeOneChildVarPredicatesPtr predicate_information) {
...
void DPBFQIterator::TryGetNext(unsigned int k) {
  // get first
  if(this->pool_.size()==0)
  {
    double cost = 0;
    ...
    {
      if(!DPBFQIterator::NextEPoolElement(k, this->fr_ow_iterators_[j], 0))
        return;
...
bool DPBFQIterator::NextEPoolElement(unsigned int k, std::shared_ptr<DPBList> &node_pointer, unsigned int index) {
  if(index == node_pointer->pool_.size())
    node_pointer->TryGetNext(k);
  if(index < node_pointer->pool_.size())
    return true;
...
void DPBFQIterator::Insert(std::shared_ptr<DPBList> FR_OW_iterator) {
 ...
 */
void DPBFQIterator::Insert(std::vector<std::shared_ptr<DPBList>> FR_OW_iterators) {
  this->fr_ow_iterators_ = std::move(FR_OW_iterators);
}


double DPBFQIterator::DeltaCost(std::shared_ptr<DPBList> &FR_OW_iterator, int index) {
  auto delta =  FR_OW_iterator->pool_[index].cost - FR_OW_iterator->pool_[index-1].cost;
  return delta;
}

void DPBFQIterator::GetResult(int i_th, std::shared_ptr<std::vector<TYPE_ENTITY_LITERAL_ID>> record,
                              NodeOneChildVarPredicatesPtr predicate_information) {
