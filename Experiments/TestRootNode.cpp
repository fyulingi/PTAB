...
const string RESULT_PREFIX = "./ExperimentResults/Root_node/";
...
extern bool global_root_strategy_enable;
...
void TestQueryWithStrategy(Database &_db, ofstream &result_file_stream, const string &db_name, const string &query_folder, const string &query_name, bool warm_up) {
    ...
    global_root_strategy_enable = true;
    if (warm_up) { // warm up
        for (auto strategy : StrategyVec) {
            global_root_strategy = strategy;
    ...
    for (auto strategy : StrategyVec) {
        global_root_strategy = strategy;
        cout << "strategy: " << RootStrategyString(strategy) << endl;
//        result_file_stream << "strategy " << RootStrategyString(strategy) << endl;
        ...
        result_file_stream << string_format("%lf,", (total_time * 1.0) / (avg_times * 1.0));
    }
    result_file_stream << endl;
    global_root_strategy_enable = false;
}

int main(int argc, char *argv[]) {
    k_value_feed = false;
    Util util;
    cout << "Test Root Node Strategy..." << endl;
    if (!util.dir_exist(RESULT_PREFIX))
        util.create_dir(RESULT_PREFIX);

    ofstream result_file_stream(RESULT_PREFIX + "root_strategy.txt");
    ...
    for (int i = 0; i < db_names.size(); ++i) {
        ...
        for (unsigned int q_i = 0; q_i < query_names[i].size(); q_i++) {
            ...
            TestQueryWithStrategy(_db, result_file_stream, db_names[i], query_folders[i], query_names[i][q_i], true);
        }
        result_file_stream << endl;
    }
    return 0;
