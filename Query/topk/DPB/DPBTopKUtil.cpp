 ...
 * @return the final DPBFRIterator
 */
DPBFRIterator *DPBUtil::BuildIteratorTree(const shared_ptr<TopKSearchPlan> tree_search_plan, TopKUtil::Env *env){
  ...
  cout << "DP-B, search node count = " << search_node_count << endl;
#endif

  // each node's descendents are OW first and FRs last
  std::vector<std::map<TYPE_ENTITY_LITERAL_ID ,std::shared_ptr<DPBFRIterator>>> descendents_FRs;
 ...
 * @return map<FQ id,pointer to DPBFQIterator>
 */
std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<DPBFQIterator>>
DPBUtil::AssemblingFrOw(set<TYPE_ENTITY_LITERAL_ID> &fq_ids,
                        ...
                        vector<std::map<TYPE_ENTITY_LITERAL_ID, shared_ptr<DPBFRIterator> >> &descendents_FRs,
                        std::vector<
                             std::map<TYPE_ENTITY_LITERAL_ID, // parent id
                                      std::pair<shared_ptr<DPBOWIterator>, // its OW
                                                NodeOneChildVarPredicatesPtr>>> // predicate correspond to the OW
                         &descendents_OWs) {
  auto ow_size = descendents_OWs.size();
  auto fr_size = descendents_FRs.size();
  auto child_type_num =  ow_size+fr_size;
  std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<DPBFQIterator>> id_fqs;
  for(auto fq_id:fq_ids)
  {
    double score=0.0;
    if(node_scores!= nullptr)
      score = (*node_scores)[fq_id];
    auto fq = make_shared<DPBFQIterator>(k, fq_id, child_type_num, score);
    // assembling fq
...
std::map<TYPE_ENTITY_LITERAL_ID, // parent id
         std::pair<std::shared_ptr<DPBOWIterator>, // its OW
                   NodeOneChildVarPredicatesPtr>> // predicate correspond to the OW
DPBUtil::GenerateOWs(int child_var, std::shared_ptr<TopKPlanUtil::TreeEdge> &tree_edges_,
...
#endif
    auto ow = make_shared<DPBOWIterator>();
...
#endif
#ifdef ITERATOR_COUNT
    search_node_count += parent_child[parent_id]->contents_->size();
#endif
  }
 ...
 */
std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<DPBFRIterator>>
DPBUtil::GenerateFRs(int child_var, std::shared_ptr<TopKPlanUtil::TreeEdge> &tree_edges_,
                     std::set<TYPE_ENTITY_LITERAL_ID>& parent_var_candidates,
                     std::set<TYPE_ENTITY_LITERAL_ID>& deleted_parents,
  ...
  std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<DPBFRIterator>> result;

  // constructing parents' FRs
  // also add predicate information into FRs
  for(auto parent_id:parent_var_candidates) {
    auto fr = make_shared<DPBFRIterator>();
    auto edges_predicates = make_shared<NodeOneChildVarPredicates>();
    for(const auto& child_id_appending_pair: *(parent_child[parent_id])->contents_) {
