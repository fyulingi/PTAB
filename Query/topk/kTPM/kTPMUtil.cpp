 ...
 * @return the final kTPMFRIterator
 */
std::shared_ptr<kTPMFRIterator> kTPMUtil::BuildIteratorTree(const shared_ptr<TopKSearchPlan> tree_search_plan, TopKUtil::Env *env){
  ...
  std::vector<std::map<TYPE_ENTITY_LITERAL_ID ,std::shared_ptr<kTPMFRIterator>>> descendents_FRs;
  ...
  auto fr = std::make_shared<kTPMFRIterator>();
 ...
 * @return map<FQ id,pointer to kTPMFQIterator>
 */
std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<kTPMFQIterator>>
kTPMUtil::AssemblingFrOw(set<TYPE_ENTITY_LITERAL_ID> &fq_ids,
                        std::shared_ptr<std::map<TYPE_ENTITY_LITERAL_ID,double>> &node_scores, int k,
                        vector<std::map<TYPE_ENTITY_LITERAL_ID, shared_ptr<kTPMFRIterator> >> &descendents_FRs,
                        std::vector<
                             std::map<TYPE_ENTITY_LITERAL_ID, // parent id
                                      std::pair<shared_ptr<kTPMOWIterator>, // its OW
                                                NodeOneChildVarPredicatesPtr>>> // predicate correspond to the OW
                         &descendents_OWs) {
  auto ow_size = descendents_OWs.size();
  auto fr_size = descendents_FRs.size();
  auto child_type_num =  ow_size+fr_size;
  std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<kTPMFQIterator>> id_fqs;
  for(auto fq_id:fq_ids)
  {
    double score=0.0;
    if(node_scores!= nullptr)
      score = (*node_scores)[fq_id];
    auto fq = make_shared<kTPMFQIterator>(k, fq_id, child_type_num, score);
    // assembling fq
...
std::map<TYPE_ENTITY_LITERAL_ID, // parent id
         std::pair<std::shared_ptr<kTPMOWIterator>, // its OW
                   NodeOneChildVarPredicatesPtr>> // predicate correspond to the OW
kTPMUtil::GenerateOWs(int child_var, std::shared_ptr<TopKPlanUtil::TreeEdge>& tree_edges_,
...
#endif
    auto ow = make_shared<kTPMOWIterator>();
 ...
 */
std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<kTPMFRIterator>>
kTPMUtil::GenerateFRs(int child_var, std::shared_ptr<TopKPlanUtil::TreeEdge>& tree_edges_,
                     std::set<TYPE_ENTITY_LITERAL_ID>& parent_var_candidates,
                     std::set<TYPE_ENTITY_LITERAL_ID>& deleted_parents,
  ...
  std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<kTPMFRIterator>> result;

  // constructing parents' FRs
  // also add predicate information into FRs
  for(auto parent_id:parent_var_candidates) {
    auto fr = make_shared<kTPMFRIterator>();
...
std::unique_ptr<Subspace> kTPMUtil::WrapStartSpace(std::shared_ptr<kTPMFRIterator> &root_fr)
{
  auto root_space = std::unique_ptr<Subspace>(new Subspace);
...
kTPMUtil::Divide(unsigned int k,
