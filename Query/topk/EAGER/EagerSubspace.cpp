...
void EagerFRIterator::GetFirst(unsigned int k) {
  return;
}
 ...
 * top EagerElement of the FR
 * @param k
 * @param fq_pointer
 */
void EagerFRIterator::Insert(unsigned int k,
                           TYPE_ENTITY_LITERAL_ID fq_id,
                           std::shared_ptr<EagerList> fq_pointer,
  ...
  if(replace)
  {
    if(this->top_element_ptr_ == nullptr) {
      ...
      this->EagerList::top_element_ptr_ = std::unique_ptr<EagerElement>(new EagerElement);
      this->EagerList::top_element_ptr_->cost =  fq_pointer->top_element_ptr_->cost;
      this->EagerList::top_element_ptr_->node =  fq_id;
      // write my self
      auto e_ptr = std::unique_ptr<EagerfrElement>(new EagerfrElement);
 ...
 */
void EagerFRIterator::GetResult(std::shared_ptr<std::vector<TYPE_ENTITY_LITERAL_ID>> record,
...
void
EagerFRIterator::SplitIntoTwoSpace(unsigned int k,
  ...
  auto top_fr = std::make_shared<EagerFRIterator>();
  auto the_other_fr = std::make_shared<EagerFRIterator>();
...
//  }
  ...
  if(!the_other_empty)
  {
    auto new_sub_space = std::unique_ptr<EagerSubspace>(new EagerSubspace);
...
EagerFRIterator::EagerFRIterator() {
  // this->type_predicates_=std::make_shared<NodeOneChildVarPredicates>();
}

void EagerFRIterator::SplitTopFq(unsigned int k,
                                double space_best_score,
                                std::vector<TYPE_ENTITY_LITERAL_ID> &record,
                                std::deque<std::shared_ptr<EagerFQIterator>> &fq_stack,
                                std::deque<unsigned int> &iterate_times,
...
void EagerFRIterator::MergeUpSubSpace(unsigned int k,
                                     double space_best_score,
                                     std::vector<TYPE_ENTITY_LITERAL_ID> &record,
                                     std::deque<std::shared_ptr<EagerFQIterator>> &fq_stack,
                                     std::deque<unsigned int> &iterate_times,
...
void EagerOWIterator::GetFirst(unsigned int k) {
}

void EagerOWIterator::Insert(unsigned int k,
                           ...
                           const std::vector<double>& scores)
{
  ...
  this->top_element_ptr_ = std::unique_ptr<EagerElement>(new EagerElement(this->pool_[0]));
}

void EagerOWIterator::RankedInsert(unsigned int k,
                            const std::vector<TYPE_ENTITY_LITERAL_ID>& ids,
                            const std::vector<double>& scores)
{
...
void EagerOWIterator::GetResult(std::shared_ptr<std::vector<TYPE_ENTITY_LITERAL_ID>> record,
                              NodeOneChildVarPredicatesPtr predicate_information) {
  auto &i_th_EagerElement = *this->top_element_ptr_;
  auto node_id = i_th_EagerElement.node;
...
void
EagerOWIterator::SplitIntoTwoSpace(unsigned int k,
                                  double space_best_score,
                                  std::vector<TYPE_ENTITY_LITERAL_ID> &record,
                                  std::deque<std::shared_ptr<EagerFQIterator>> &fq_stack,
                                  std::deque<unsigned int> &iterate_times,
  ...
  auto the_other_ow = std::make_shared<EagerOWIterator>();

  bool no_coefficient = std::all_of(this->pool_.cbegin(), this->pool_.cend(),
                                    [](const EagerElement &ele){ return ele.cost== 0.0; });
  ...
  if(!the_other_empty) {
    if (no_coefficient)
      ...
      the_other_ow->RankedInsert(k, ids, scores);
  }
...
void
EagerOWIterator::MergeUpSubSpace(unsigned int k,
                                double space_best_score,
                                std::vector<TYPE_ENTITY_LITERAL_ID> &record,
                                std::deque<std::shared_ptr<EagerFQIterator>> &fq_stack,
                                std::deque<unsigned int> &iterate_times,
...
void EagerFQIterator::GetFirst(unsigned int k) {
  double cost = 0;
  for (unsigned int j = 0; j < this->fr_ow_iterators_.size(); j++) {
    cost += this->fr_ow_iterators_[j]->top_element_ptr_->cost;
  }

  auto ele_ptr = std::unique_ptr<EagerElement>(new EagerElement);
  ele_ptr->cost = cost;
  this->top_element_ptr_ = std::move(ele_ptr);
}

void EagerFQIterator::Insert(std::shared_ptr<EagerList> FR_OW_iterator) {
 ...
 */
void EagerFQIterator::Insert(std::vector<std::shared_ptr<EagerList>> FR_OW_iterators) {
  this->fr_ow_iterators_ = std::move(FR_OW_iterators);
}

void EagerFQIterator::GetResult(std::shared_ptr<std::vector<TYPE_ENTITY_LITERAL_ID>> record,
                               NodeOneChildVarPredicatesPtr predicate_information) {
...
void
EagerFQIterator::SplitIntoTwoSpace(unsigned int k,
                                  double space_best_score,
                                  std::vector<TYPE_ENTITY_LITERAL_ID> &record,
                                  std::deque<std::shared_ptr<EagerFQIterator>> &fq_stack,
                                  std::deque<unsigned int> &iterate_times,
                                  EagerSpaceHeap *collection) {
  record.push_back(this->node_id_);
  fq_stack.push_back(this->shared_from_base<EagerFQIterator>());
...
void EagerFQIterator::MergeUpSubSpace(unsigned int k,
                                    double space_best_score,
                                    std::vector<TYPE_ENTITY_LITERAL_ID> &record,
                                    std::deque<std::shared_ptr<EagerFQIterator>> &fq_stack,
                                    std::deque<unsigned int> &iterate_times,
