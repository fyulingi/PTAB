...
void TopkStrategyPrint(TopKStrategy topk_stra) {
  switch(topk_stra) {
    case TopKStrategy::DP_B:
      cout << "DP-B";
      break;
    case TopKStrategy::DP_P:
      cout << "DP-P";
      break;
    case TopKStrategy::K_TPM:
      cout << "kTPM";
      break;
    case TopKStrategy::SAE:
      cout << "SAE";
      break;
    case TopKStrategy::DFS_OLD:
      cout << "DFS-OLD";
      break;
    case TopKStrategy::PTAB:
      cout << "PTAB";
      break;
    case TopKStrategy::TAKE_ALL:
      cout << "Take-all";
      break;
    case TopKStrategy::TAKE2:
      cout << "Take2";
      break;
    case TopKStrategy::EAGER:
      cout << "EAGER";
      break;
    case TopKStrategy::RankAfterMatching:
      cout << "Naive";
      break;
    default:
...
3. ./gquery db_folder query_path                               load query from given path fro given database\n\
4. ./gquery db_folder                                          load the given database and open console\n\
5. ./gquery db_folder query_path result_file [top-k_method]    for top-k subgraph matching\
=============================================================================*/\n");
}
...
//#endif
	...
	if (argc >= 3)
	{
		...
		{
		  TopKStrategy method;
		  if(argv[4] == string("DP-B"))
            method = TopKStrategy::DP_B;
          if(argv[4] == string("DP-P"))
            method = TopKStrategy::DP_P;
          if(argv[4] == string("kTPM"))
            method = TopKStrategy::K_TPM;
          if(argv[4] == string("DFS-OLD"))
            method = TopKStrategy::DFS_OLD;
          if(argv[4] == string("PTAB"))
            method = TopKStrategy::PTAB;
          if(argv[4] == string("Take-all"))
            method = TopKStrategy::TAKE_ALL;
          if(argv[4] == string("Take2"))
            method = TopKStrategy::TAKE2;
          if(argv[4] == string("Eager"))
            method = TopKStrategy::EAGER;
          if(argv[4] == string("SAE"))
            method = TopKStrategy::SAE;
          if(argv[4] == string("Naive"))
            method = TopKStrategy::RankAfterMatching;
          cout << "Top-k method: ";
          TopkStrategyPrint(method);
          cout << endl;

          ret = _db.query(query, _rs, ofp, true, false, nullptr,method);
		}
		else
		  ret = _db.query(query, _rs, ofp, true, false, nullptr, TopKStrategy::PTAB);
