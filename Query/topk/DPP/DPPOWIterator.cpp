...
void DPPOWIterator::TryGetNext(unsigned int k) {
  if(this->valid_num<k && this->valid_num<this->pool_.size())
    this->valid_num++;
}

void DPPOWIterator::Insert(unsigned int k,
 ...
 * @param ids
 */
void DPPOWIterator::Insert(unsigned int k,const std::vector<TYPE_ENTITY_LITERAL_ID> &ids) {
  pool_.reserve(k);
  ...
  valid_num = this->pool_.size();
}

void DPPOWIterator::GetResult(int i_th, std::shared_ptr<std::vector<TYPE_ENTITY_LITERAL_ID>> record,
