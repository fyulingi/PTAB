...
#include "TakeAllUtil.h"


/**
 * Build Iterators for top k search.
 * @return the final TakeAllFRIterator
 */
std::shared_ptr<TakeAllFRIterator> TakeAllUtil::BuildIteratorTree(const shared_ptr<TopKSearchPlan> tree_search_plan, TopKUtil::Env *env){
  ...
  std::vector<std::map<TYPE_ENTITY_LITERAL_ID ,std::shared_ptr<TakeAllFRIterator>>> descendents_FRs;
  ...
  auto fr = std::make_shared<TakeAllFRIterator>();
 ...
 * @return map<FQ id,pointer to TakeAllFQIterator>
 */
std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<TakeAllFQIterator>>
TakeAllUtil::AssemblingFrOw(set<TYPE_ENTITY_LITERAL_ID> &fq_ids,
                        std::shared_ptr<std::map<TYPE_ENTITY_LITERAL_ID,double>> &node_scores, int k,
                        vector<std::map<TYPE_ENTITY_LITERAL_ID, shared_ptr<TakeAllFRIterator> >> &descendents_FRs,
                        std::vector<
                             std::map<TYPE_ENTITY_LITERAL_ID, // parent id
                                      std::pair<shared_ptr<TakeAllOWIterator>, // its OW
                                                NodeOneChildVarPredicatesPtr>>> // predicate correspond to the OW
                         &descendents_OWs) {
  auto ow_size = descendents_OWs.size();
  auto fr_size = descendents_FRs.size();
  auto child_type_num =  ow_size+fr_size;
  std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<TakeAllFQIterator>> id_fqs;
  for(auto fq_id:fq_ids)
  {
    double score=0.0;
    if(node_scores!= nullptr)
      score = (*node_scores)[fq_id];
    auto fq = make_shared<TakeAllFQIterator>(k, fq_id, child_type_num, score);
    // assembling fq
...
std::map<TYPE_ENTITY_LITERAL_ID, // parent id
         std::pair<std::shared_ptr<TakeAllOWIterator>, // its OW
                   NodeOneChildVarPredicatesPtr>> // predicate correspond to the OW
TakeAllUtil::GenerateOWs(int child_var, std::shared_ptr<TopKPlanUtil::TreeEdge>& tree_edges_,
...
#endif
    auto ow = make_shared<TakeAllOWIterator>();
 ...
 */
std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<TakeAllFRIterator>>
TakeAllUtil::GenerateFRs(int child_var, std::shared_ptr<TopKPlanUtil::TreeEdge>& tree_edges_,
                     std::set<TYPE_ENTITY_LITERAL_ID>& parent_var_candidates,
                     std::set<TYPE_ENTITY_LITERAL_ID>& deleted_parents,
  ...
  std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<TakeAllFRIterator>> result;

  // constructing parents' FRs
  // also add predicate information into FRs
  for(auto parent_id:parent_var_candidates) {
    auto fr = make_shared<TakeAllFRIterator>();
...
std::unique_ptr<TakeAllSubspace> TakeAllUtil::WrapStartSpace(std::shared_ptr<TakeAllFRIterator> &root_fr)
{
  auto root_space = std::unique_ptr<TakeAllSubspace>(new TakeAllSubspace);
...
TakeAllUtil::Divide(unsigned int k,
