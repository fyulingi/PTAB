...
#include "DPBanyTopKUtil.h"


/**
 * Build Iterators for top k search.
 * @return the final DPBanyFR
 */
DPBanyFR *DPBanyUtil::BuildIteratorTree(const shared_ptr<TopKSearchPlan> tree_search_plan, TopKUtil::Env *env){
  ...
  std::vector<std::map<TYPE_ENTITY_LITERAL_ID ,std::shared_ptr<DPBanyFR>>> descendents_FRs;
 ...
 * @return map<FQ id,pointer to DPBanyFQ>
 */
std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<DPBanyFQ>>
DPBanyUtil::AssemblingFrOw(set<TYPE_ENTITY_LITERAL_ID> &fq_ids,
                        std::shared_ptr<std::map<TYPE_ENTITY_LITERAL_ID,double>> &node_scores, int k,
                        vector<std::map<TYPE_ENTITY_LITERAL_ID, shared_ptr<DPBanyFR> >> &descendents_FRs,
                        std::vector<
                             std::map<TYPE_ENTITY_LITERAL_ID, // parent id
                                      std::pair<shared_ptr<DPBanyOW>, // its OW
                                                NodeOneChildVarPredicatesPtr>>> // predicate correspond to the OW
                         &descendents_OWs) {
  auto ow_size = descendents_OWs.size();
  auto fr_size = descendents_FRs.size();
  auto child_type_num =  ow_size+fr_size;
  std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<DPBanyFQ>> id_fqs;
  for(auto fq_id:fq_ids)
  {
    double score=0.0;
    if(node_scores!= nullptr)
      score = (*node_scores)[fq_id];
    auto fq = make_shared<DPBanyFQ>(k, fq_id, child_type_num, score);
    // assembling fq
...
std::map<TYPE_ENTITY_LITERAL_ID, // parent id
         std::pair<std::shared_ptr<DPBanyOW>, // its OW
                   NodeOneChildVarPredicatesPtr>> // predicate correspond to the OW
DPBanyUtil::GenerateOWs(int child_var, std::shared_ptr<TopKPlanUtil::TreeEdge> &tree_edges_,
 ...
 */
std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<DPBanyFR>>
DPBanyUtil::GenerateFRs(int child_var, std::shared_ptr<TopKPlanUtil::TreeEdge> &tree_edges_,
                     std::set<TYPE_ENTITY_LITERAL_ID>& parent_var_candidates,
                     std::set<TYPE_ENTITY_LITERAL_ID>& deleted_parents,
  ...
  std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<DPBanyFR>> result;

  // constructing parents' FRs
  // also add predicate information into FRs
  for(auto parent_id:parent_var_candidates) {
    auto fr = make_shared<DPBanyFR>();
