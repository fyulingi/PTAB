 ...
 * @return the final SAEFRIterator
 */
std::shared_ptr<SAEFRIterator> SAEUtil::BuildIteratorTree(const shared_ptr<TopKSearchPlan> tree_search_plan, TopKUtil::Env *env){
  ...
  std::vector<std::map<TYPE_ENTITY_LITERAL_ID ,std::shared_ptr<SAEFRIterator>>> descendents_FRs;
  ...
  auto fr = std::make_shared<SAEFRIterator>();
 ...
 * @return map<FQ id,pointer to SAEFQIterator>
 */
std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<SAEFQIterator>>
SAEUtil::AssemblingFrOw(set<TYPE_ENTITY_LITERAL_ID> &fq_ids,
                        std::shared_ptr<std::map<TYPE_ENTITY_LITERAL_ID,double>> &node_scores, int k,
                        vector<std::map<TYPE_ENTITY_LITERAL_ID, shared_ptr<SAEFRIterator> >> &descendents_FRs,
                        std::vector<
                             std::map<TYPE_ENTITY_LITERAL_ID, // parent id
                                      std::pair<shared_ptr<SAEOWIterator>, // its OW
                                                NodeOneChildVarPredicatesPtr>>> // predicate correspond to the OW
                         &descendents_OWs) {
  auto ow_size = descendents_OWs.size();
  auto fr_size = descendents_FRs.size();
  auto child_type_num =  ow_size+fr_size;
  std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<SAEFQIterator>> id_fqs;
  for(auto fq_id:fq_ids)
  {
    double score=0.0;
    if(node_scores!= nullptr)
      score = (*node_scores)[fq_id];
    auto fq = make_shared<SAEFQIterator>(k, fq_id, child_type_num, score);
    // assembling fq
...
std::map<TYPE_ENTITY_LITERAL_ID, // parent id
         std::pair<std::shared_ptr<SAEOWIterator>, // its OW
                   NodeOneChildVarPredicatesPtr>> // predicate correspond to the OW
SAEUtil::GenerateOWs(int child_var, std::shared_ptr<TopKPlanUtil::TreeEdge>& tree_edges_,
...
#endif
    auto ow = make_shared<SAEOWIterator>();
 ...
 */
std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<SAEFRIterator>>
SAEUtil::GenerateFRs(int child_var, std::shared_ptr<TopKPlanUtil::TreeEdge>& tree_edges_,
                     std::set<TYPE_ENTITY_LITERAL_ID>& parent_var_candidates,
                     std::set<TYPE_ENTITY_LITERAL_ID>& deleted_parents,
  ...
  std::map<TYPE_ENTITY_LITERAL_ID,std::shared_ptr<SAEFRIterator>> result;

  // constructing parents' FRs
  // also add predicate information into FRs
  for(auto parent_id:parent_var_candidates) {
    auto fr = make_shared<SAEFRIterator>();
...
std::unique_ptr<SpaceSAE> SAEUtil::WrapStartSpace(std::shared_ptr<SAEFRIterator> &root_fr)
{
  std::unique_ptr<std::stack<std::shared_ptr<SAEList>>>
      fr_ow_iterator( new  std::stack<std::shared_ptr<SAEList>>());
  fr_ow_iterator->push(root_fr);
  return std::unique_ptr<SpaceSAE>(new SpaceSAE(std::vector<TYPE_ENTITY_LITERAL_ID>(),
                                                0,
                                                std::move(fr_ow_iterator),
                                                root_fr->top_element_ptr_->cost));
}

// 从上到下分裂的入口
std::shared_ptr<vector<TYPE_ENTITY_LITERAL_ID>>
SAEUtil::Divide(unsigned int k,
  ...
  auto root_fr = s_ptr->fr_ow_iterators_->top();
  root_fr->SplitIntoTwoSpace(k,s_ptr->best_score_,*result_record,std::move(s_ptr->fr_ow_iterators_),
                             s_ptr->iterated_items_,collection);
