...
#ifndef _UTIL_UTIL_H
#define _UTIL_UTIL_H
#define ITERATOR_COUNT
#define TOPK_SUPPORT
#define NEW_EXPLORE_STRATEGY
#define COST_MODEL
//basic macros and types are defined here, including common headers 
#define D_HOP 4
#define D_HOP_LOG 3 // log_(2)(4) + 1
#include <stdio.h>
#include <stdlib.h>
#include <stdbool.h>
#include <limits.h>
...
public:
	...
	static void add_transactionlog(std::string db_name, std::string user, std::string TID,  std::string begin_time, std::string status = "RUNNING",  std::string end_time = "inf");
	static int delete_transactionlog(std::string db_name, std::string TID);
	static void update_transactionlog(std::string db_name, std::string status, std::string end_time);
