...
void kTPMFRIterator::GetFirst(unsigned int k) {
  return;
}
 ...
 * top kTPMElement of the FR
 * @param k
 * @param fq_pointer
 */
void kTPMFRIterator::Insert(unsigned int k,
                           TYPE_ENTITY_LITERAL_ID fq_id,
                           std::shared_ptr<kTPMList> fq_pointer,
  ...
  if(replace)
  {
    if(this->top_element_ptr_ == nullptr) {
      ...
      this->kTPMList::top_element_ptr_ = std::unique_ptr<kTPMElement>(new kTPMElement);
      this->kTPMList::top_element_ptr_->cost =  fq_pointer->top_element_ptr_->cost;
      this->kTPMList::top_element_ptr_->node =  fq_id;
      // write my self
      auto e_ptr = std::unique_ptr<kTPMfrElement>(new kTPMfrElement);
 ...
 */
void kTPMFRIterator::GetResult(std::shared_ptr<std::vector<TYPE_ENTITY_LITERAL_ID>> record,
...
void
kTPMFRIterator::SplitIntoTwoSpace(unsigned int k,
  ...
  auto top_fr = std::make_shared<kTPMFRIterator>();
  auto the_other_fr = std::make_shared<kTPMFRIterator>();
...
//  }
  ...
  if(!the_other_empty)
  {
    auto new_sub_space = std::unique_ptr<Subspace>(new Subspace);
...
kTPMFRIterator::kTPMFRIterator() {
  // this->type_predicates_=std::make_shared<NodeOneChildVarPredicates>();
}

void kTPMFRIterator::SplitTopFq(unsigned int k,
                                double space_best_score,
                                std::vector<TYPE_ENTITY_LITERAL_ID> &record,
                                std::deque<std::shared_ptr<kTPMFQIterator>> &fq_stack,
                                std::deque<unsigned int> &iterate_times,
...
void kTPMFRIterator::MergeUpSubSpace(unsigned int k,
                                     double space_best_score,
                                     std::vector<TYPE_ENTITY_LITERAL_ID> &record,
                                     std::deque<std::shared_ptr<kTPMFQIterator>> &fq_stack,
                                     std::deque<unsigned int> &iterate_times,
...
void kTPMOWIterator::GetFirst(unsigned int k) {
}

void kTPMOWIterator::Insert(unsigned int k,
                           ...
                           const std::vector<double>& scores)
{
  ...
  this->top_element_ptr_ = std::unique_ptr<kTPMElement>(new kTPMElement(this->pool_[0]));
}

void kTPMOWIterator::RankedInsert(unsigned int k,
                            const std::vector<TYPE_ENTITY_LITERAL_ID>& ids,
                            const std::vector<double>& scores)
{
...
void kTPMOWIterator::GetResult(std::shared_ptr<std::vector<TYPE_ENTITY_LITERAL_ID>> record,
                              NodeOneChildVarPredicatesPtr predicate_information) {
  auto &i_th_kTPMElement = *this->top_element_ptr_;
  auto node_id = i_th_kTPMElement.node;
...
void
kTPMOWIterator::SplitIntoTwoSpace(unsigned int k,
                                  double space_best_score,
                                  std::vector<TYPE_ENTITY_LITERAL_ID> &record,
                                  std::deque<std::shared_ptr<kTPMFQIterator>> &fq_stack,
                                  std::deque<unsigned int> &iterate_times,
  ...
  auto the_other_ow = std::make_shared<kTPMOWIterator>();

//  bool no_coefficient = std::all_of(this->pool_.cbegin(), this->pool_.cend(),
//                                    [](const kTPMElement &ele){ return ele.cost== 0.0; });
//
//
...
void
kTPMOWIterator::MergeUpSubSpace(unsigned int k,
                                double space_best_score,
                                std::vector<TYPE_ENTITY_LITERAL_ID> &record,
                                std::deque<std::shared_ptr<kTPMFQIterator>> &fq_stack,
                                std::deque<unsigned int> &iterate_times,
...
void kTPMFQIterator::GetFirst(unsigned int k) {
  double cost = 0;
  for (unsigned int j = 0; j < this->fr_ow_iterators_.size(); j++) {
    cost += this->fr_ow_iterators_[j]->top_element_ptr_->cost;
  }

  auto ele_ptr = std::unique_ptr<kTPMElement>(new kTPMElement);
  ele_ptr->cost = cost;
  this->top_element_ptr_ = std::move(ele_ptr);
}

void kTPMFQIterator::Insert(std::shared_ptr<kTPMList> FR_OW_iterator) {
 ...
 */
void kTPMFQIterator::Insert(std::vector<std::shared_ptr<kTPMList>> FR_OW_iterators) {
  this->fr_ow_iterators_ = std::move(FR_OW_iterators);
}

void kTPMFQIterator::GetResult(std::shared_ptr<std::vector<TYPE_ENTITY_LITERAL_ID>> record,
                               NodeOneChildVarPredicatesPtr predicate_information) {
...
void
kTPMFQIterator::SplitIntoTwoSpace(unsigned int k,
                                  double space_best_score,
                                  std::vector<TYPE_ENTITY_LITERAL_ID> &record,
                                  std::deque<std::shared_ptr<kTPMFQIterator>> &fq_stack,
                                  std::deque<unsigned int> &iterate_times,
                                  SpaceHeap *collection) {
  record.push_back(this->node_id_);
  fq_stack.push_back(this->shared_from_base<kTPMFQIterator>());
...
void kTPMFQIterator::MergeUpSubSpace(unsigned int k,
                                    double space_best_score,
                                    std::vector<TYPE_ENTITY_LITERAL_ID> &record,
                                    std::deque<std::shared_ptr<kTPMFQIterator>> &fq_stack,
                                    std::deque<unsigned int> &iterate_times,
